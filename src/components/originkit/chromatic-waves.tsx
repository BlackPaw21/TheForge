...
const MAX_RENDER_TARGET_SIZE = 4096;
...
export default function DottedBg2({
  ...
  useEffect(() => {
    ...
    // ogl's Renderer crashes when WebGL is unavailable ("Cannot set
    // properties of null (setting 'renderer')") — bail out gracefully and
    // leave the background static instead of throwing on page load.
    const probe = document.createElement("canvas");
    if (!(probe.getContext("webgl") || probe.getContext("webgl2"))) return;
    ...
    const doResize = () => {
      ...
      if (renderTargetRef.current && renderTargetRef.current.setSize) {
        renderTargetRef.current.setSize(
          Math.min(gl.canvas.width, MAX_RENDER_TARGET_SIZE),
          Math.min(gl.canvas.height, MAX_RENDER_TARGET_SIZE)
        );
      }
    ...
    const renderTarget = new OglRenderTarget(gl, {
      width: Math.min(gl.canvas.width, MAX_RENDER_TARGET_SIZE),
      height: Math.min(gl.canvas.height, MAX_RENDER_TARGET_SIZE),
    });
